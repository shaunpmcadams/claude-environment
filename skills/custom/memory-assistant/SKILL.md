...
description: >
  Comprehensive Claude memory advisor — full analysis with Layer 2 state view, Layer 1 accuracy
  pulse, intent coverage analysis (Covered/Missing/Drifted/Wrong-layer), conversation-aware
  suggestions for new facts worth remembering, and prioritized recommendations with inline
  execution. Use this skill when the user wants a thorough memory review, asks if their memory
  is aligned with their intent, wants to know what's worth remembering from a conversation,
  asks for memory recommendations, or wants proactive suggestions about what to persist.
  Also trigger on "memory assistant", "review my memory", "is my memory healthy", "what should
  I remember", "memory advisory", "analyze my memory", "memory recommendations", "help me with
  my memory", "should I remember this". This is the 3-5 minute advisory. For a quick 30-second
  check, use memory-status instead.
---

# Memory Assistant — Comprehensive Advisory

Full memory analysis: current state + intent coverage + conversation-aware suggestions + prioritized recommendations with inline execution. Takes 3-5 minutes.

For a quick 30-second status check, use `/memory-status` instead.
...
- **`memory_user_edits` tool is available** — Call it with command `"view"` as a probe. If the tool is unavailable or returns an error, stop and explain: "The memory_user_edits tool isn't available in this session. This skill requires it to read and edit Layer 2 state."
...
The Layer 1 Pulse is a best-effort scan from in-session context. Layer 1 has ~100 entries; only what is visible or inferable here is assessed. For a complete audit, use `memory-dream.md` after a health check.
...
A good suggestion is a single, stable fact that applies broadly across sessions. Not project status, not formatting preferences (those belong in Preferences/Styles), not something Claude already handles accurately via Layer 1.
